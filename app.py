...
def buscar_imagenes(query, cantidad=4):
    ...
    try:
        ...
        # 🔥 importante: detectar errores HTTP reales
        if response.status_code != 200:
            st.error(f"Error API Unsplash: {response.status_code}")
            return []
        ...
        st.error("Timeout en la API de imágenes")
        return []
